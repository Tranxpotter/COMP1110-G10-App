...
import { deleteRecord, executeSqlAsync, fetchAllCategories, fetchAllRecipients, fetchRecordsWithFilters, initTables, updateRecord } from '../components/dbClient'
  ...
  const loadRecords = useCallback(async (config = filterConfig) => {
    try {
      ...
      const [rows, categories, recipients] = await Promise.all([
        fetchRecordsWithFilters(config),
        fetchAllCategories(),
        fetchAllRecipients(),
      ])
